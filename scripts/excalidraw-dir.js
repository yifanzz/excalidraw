...
function explode(inputFile, outputDir) {
  ...
  console.log(
    `Exploded ${inputFile} → ${outputDir}/ (${assetCount} asset${assetCount !== 1 ? "s" : ""})`,
  );
}
...
function pack(inputDir, outputFile) {
  ...
  if (!outputFile) {
    outputFile = inputDir.replace(/\/?$/, "").replace(/\.d$/, "") + ".excalidraw";
  ...
  console.log(
    `Packed ${inputDir}/ → ${outputFile} (${assetCount} asset${assetCount !== 1 ? "s" : ""})`,
  );
}
